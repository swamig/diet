---
title: 'Advanced Control'
section: 'advanced-control'
type: 'guide'
priority: 'reference'
---

# Advanced Control

[Back to Manual](../README.md)

Use this section when the base protocol is not enough and you need tighter control over load, spacing, or conflicts between ingredients and supplements.

## Topics

- [Antimicrobial Stacking Framework](antimicrobial-stacking.md)
- [Blueprint Product Integration (Bryan Johnson)](blueprint-integration.md)
- [Circadian Sleep Architecture & Autonomic Tone](sleep-architecture.md)
- [Glycemic Dynamics & Carbohydrate Sequencing](glycemic-sequencing.md)
- [Heavy Metal Mitigation](heavy-metals.md)
- [Hormetic Resistance & Biological Tolerance Avoidance](hormetic-resistance.md)
- [Liver Load Spacing and Detox Pathways](liver-load.md)
- [Metabolic Oncology & Chemoprevention](oncology-chemoprevention.md)
- [Omega Load Balancing](omega-balance.md)
- [Oxalate Physiology and Mineral Pairing](oxalate-management.md)
- [Sulfur Cycle Optimization](sulfur-management.md)
- [Supplement Cycling & Management](supplement-cycling.md)
- [Advanced Training & Thermal Hormesis Integration](training-hormesis-integration.md)

## Typical Questions This Section Answers

- How often should high-intensity spices appear?
- When do cacao, flavored whey, or other oxalate sources need spacing?
- When is sulfur support helpful versus excessive?
- How do supplements fit without distorting the food-first structure?

## Related Sections

- Use [Ingredients & Monographs](../02-ingredients/README.md) for the ingredient-specific mechanism.
- Use [Monitoring](../06-monitoring/README.md) and [Logs](../logs/README.md) to see whether a control strategy actually changed outcomes.
