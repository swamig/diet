---
title: 'Black Pepper (Bioavailability Amplifier)'
section: 'ingredients'
type: 'monograph'
ingredient: 'black-pepper'
priority: 'core'
---

# Black Pepper (Bioavailability Amplifier)

## Active Compounds

*   **Piperine**

## Mechanism of Action

Piperine:

*   Inhibits **glucuronidation**
*   Enhances absorption of **curcumin**
*   Increases intestinal permeability transiently
*   May modestly influence thermogenesis

## Dose

*   1/4 teaspoon (~0.5 grams) daily.

## Stacking Rules

Black pepper:

*   **Always paired with turmeric**
*   Does not require rotation
*   Should not be used in excessive quantity (to avoid gastric irritation)

## Protocol Role

- Black pepper is the bioavailability amplifier for the turmeric-centered anti-inflammatory layer.
- Its main role in this manual is to be added fresh at the point of use rather than treated as generic seasoning.

## Evidence Status

- **General physiology:** The compound and mechanism descriptions on this page are used as directional context for how the ingredient may fit into the protocol.
- **Protocol-specific rules:** The exact dose, timing, pairings, weekly caps, and "mandatory" language on this page are operating rules specific to this manual.

## Used In

- [Inflammatory Signaling Control](../01-foundations/02-inflammatory-control.md)
- [High Signaling Container System 🧪](../05-practical/container-system.md)
- [90-Minute Sunday Batch Prep System](../05-practical/sunday-prep.md)
- [Daily Execution](../03-execution/README.md)

## Conflicts

- Keep black pepper paired with turmeric in the core stack rather than treating it as a free dose escalator.
- Pre-grinding it into weekly jars works against the freshness strategy used in this manual.


## Culinary Integration (Recipes)
This ingredient is functionally integrated into the following protocol meals:
- [Vegan Protein Patties (Spice Sear)](../05-practical/vegan-protein-patties.md)
- [Master Dressing (Sunday Prep)](../05-practical/sunday-prep.md#block-6-dressing-master-batch-5-min)

## Related Pages

- [Turmeric (Foundational Anti-Inflammatory Anchor)](turmeric.md)
- [Inflammatory Signaling Control](../01-foundations/02-inflammatory-control.md)
- [High Signaling Container System 🧪](../05-practical/container-system.md)
- [Ingredient Conflict Matrix](../07-appendices/ingredient-conflict-matrix.md)
