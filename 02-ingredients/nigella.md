---
title: 'Nigella (Black Jeera) - Metabolic Modulation Agent'
section: 'ingredients'
type: 'monograph'
ingredient: 'nigella'
priority: 'rotation'
---

# Nigella (Black Jeera) - Metabolic Modulation Agent

## Active Compounds

*   **Thymoquinone**

## Mechanism of Action

Thymoquinone influences:

*   **Glucose regulation**
*   **Inflammatory modulation**
*   **Mitochondrial protection**
*   **Mild antimicrobial effects**

Nigella is more pharmacologically active than cumin. Therefore it is not daily.

## Dose

*   1/4 teaspoon (~0.7 grams)
*   **Used 3x per week.**

## Stacking Rules

Nigella is:

*   **Not stacked with high clove days**
*   **Not stacked with excessive antimicrobial load**
*   Rotated to prevent microbiome suppression

## Protocol Role

- Nigella is a controlled rotational spice used for antimicrobial and metabolic modulation.
- It is intentionally stronger than the daily base layer and therefore managed more carefully.

## Evidence Status

- **General physiology:** The compound and mechanism descriptions on this page are used as directional context for how the ingredient may fit into the protocol.
- **Protocol-specific rules:** The exact dose, timing, pairings, weekly caps, and "mandatory" language on this page are operating rules specific to this manual.

## Used In

- [Gut Microbiome Diversification](../01-foundations/03-microbiome-hormesis.md)
- [Antimicrobial Stacking Framework](../04-advanced-control/antimicrobial-stacking.md)
- [Week 2 Overview](../03-execution/week-02/README.md)
- [Week 3 Overview](../03-execution/week-03/README.md)

## Conflicts

- Do not stack nigella at high frequency with clove or ajwain inside this protocol.
- Treating nigella as a daily background spice works against the intended rotational use.


## Culinary Integration (Recipes)
This ingredient is functionally integrated into the following protocol meals:
- [Vegan Protein Patties (Spice Sear Variant)](../05-practical/vegan-protein-patties.md)

## Related Pages

- [Ajwain (Carom Seed) - Digestive Activation Agent](ajwain.md)
- [Clove - High-Intensity Antimicrobial and Antioxidant Agent](clove.md)
- [Antimicrobial Stacking Framework](../04-advanced-control/antimicrobial-stacking.md)
- [Week 3 Overview](../03-execution/week-03/README.md)
