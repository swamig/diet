---
title: 'Cilantro - Heavy Metal Chelation and Antioxidant Support'
section: 'ingredients'
type: 'monograph'
ingredient: 'cilantro'
priority: 'core'
---

# Cilantro - Heavy Metal Chelation and Antioxidant Support

## Active Compounds

*   **Linalool**
*   **Quercetin**
*   **Dodecenal** (antimicrobial)

## Mechanism of Action

Cilantro [[Laribi et al., 2015]](../../07-appendices/master-citations.md#10-antioxidants-polyphenols--vascular-signaling):

*   **Mobilizes heavy metals** (mild chelation effect via linalool and dodecenal compounds)
*   **Supports liver detoxification**
*   **Reduces oxidative stress** (quercetin-mediated)
*   **Enhances digestion** (linalool carminative and anti-spasmodic activity on smooth muscle)

## Dose

*   **Generous handful daily (1/4 - 1/2 cup chopped)**.

## Stacking Rules

*   **Included in salads or as garnish daily.**
*   **Pairs well with microgreens.**
*   **Do not cook heavily; add fresh.**

## Protocol Role

- Cilantro is a daily fresh-herb anchor that keeps the salad layer feeling alive and consistent.
- It is used for freshness, garnish density, and the manual’s broader heavy-metal and herb-layer logic.

## Evidence Status

- **General physiology:** The compound and mechanism descriptions on this page are used as directional context for how the ingredient may fit into the protocol.
- **Protocol-specific rules:** The exact dose, timing, pairings, weekly caps, and "mandatory" language on this page are operating rules specific to this manual.

## Used In

- [Heavy Metal Mitigation (Cacao)](../04-advanced-control/heavy-metals.md)
- [100% Optimized Indian High-Protein "Chicken" Patties](../05-practical/vegan-protein-patties.md)
- [Consolidated Master Shopping List](../05-practical/shopping-list-master.md)
- [Daily Execution](../03-execution/README.md)

## Conflicts

- Treating cilantro as optional garnish weakens one of the daily anchor layers used across the cycle.
- Poor storage makes this ingredient feel harder to keep than it needs to be.


## Culinary Integration (Recipes)
This ingredient is functionally integrated into the following protocol meals:
- [14:30 Salad Layer (Chelation Anchor)](../05-practical/daily-salad-layer.md)
- [Performance Refried Beans](../05-practical/performance-refried-beans.md)

## Related Pages

- [Heavy Metal Mitigation (Cacao)](../04-advanced-control/heavy-metals.md)
- [Consolidated Master Shopping List](../05-practical/shopping-list-master.md)
- [Fridge Layout Optimization](../05-practical/fridge-storage.md)

## References

- [[Laribi et al., 2015]](../../07-appendices/master-citations.md#10-antioxidants-polyphenols--vascular-signaling) — Linalool (dominant volatile in coriander/cilantro) carminative and anti-spasmodic activity in smooth muscle models.
