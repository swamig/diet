---
title: 'Cacao - Vascular Polyphenol Layer'
section: 'ingredients'
type: 'monograph'
ingredient: 'cacao'
priority: 'rotation'
---

# Cacao - Vascular Polyphenol Layer

## Active Compounds

*   **Epicatechin**
*   **Theobromine**
*   **Flavanols**

## Mechanism of Action

Cacao [[Johnson et al., 2011]](../../07-appendices/master-citations.md#10-antioxidants-polyphenols--vascular-signaling):

*   Enhances **nitric oxide signaling** (epicatechin-mediated)
*   Improves **endothelial function**
*   Modulates **oxidative stress** (anthocyanin flavanols)
*   **Mild stimulant effect** (theobromine)
*   Provides **magnesium**

## Risks and Constraints

*   **Oxalate content**
*   Potential heavy metal contamination

## Dose

*   1 tbsp, 2–3x weekly.
*   **Not daily.**
*   **Rotated to prevent cumulative oxalate stacking.**

## Protocol Role

- Cacao is a vascular-support rotator rather than a daily background food in this protocol.
- It is deliberately placed in the recovery context where its stimulant and oxalate tradeoffs can be managed.

## Stacking Rules

- Keep cacao to the planned 2-3x weekly rhythm used in the recovery layer.
- Treat it as a spaced rotator, not a default every-day add-on.

## Evidence Status

- **General physiology:** The compound and mechanism descriptions on this page are used as directional context for how the ingredient may fit into the protocol.
- **Protocol-specific rules:** The exact dose, timing, pairings, weekly caps, and "mandatory" language on this page are operating rules specific to this manual.

## Used In

- [Oxalate Physiology and Mineral Pairing](../04-advanced-control/oxalate-management.md)
- [Heavy Metal Mitigation (Cacao)](../04-advanced-control/heavy-metals.md)
- [Supplement Cycling & Management](../04-advanced-control/supplement-cycling.md)
- [Week 3 Overview](../03-execution/week-03/README.md)

## Conflicts

- Do not combine cacao nibs with chocolate whey on the same day in this protocol.
- Daily cacao use or removing the dairy / calcium context works against the intended spacing logic.


## Culinary Integration (Recipes)
This ingredient is functionally integrated into the following protocol meals:
- [17:50 Recovery Bowl](../03-execution/week-01/day-01.md#recovery-bowl)

## Related Pages

- [Oxalate Physiology and Mineral Pairing](../04-advanced-control/oxalate-management.md)
- [Heavy Metal Mitigation (Cacao)](../04-advanced-control/heavy-metals.md)
- [Supplement Cycling & Management](../04-advanced-control/supplement-cycling.md)
- [Ingredient Conflict Matrix](../07-appendices/ingredient-conflict-matrix.md)

## References

- [[Johnson et al., 2011]](../../07-appendices/master-citations.md#10-antioxidants-polyphenols--vascular-signaling) — Anthocyanins and flavanols in cacao modulate oxidative stress and support neurocognitive function.
