---
title: 'Sesame Seed - Mineral Density Layer'
section: 'ingredients'
type: 'monograph'
ingredient: 'sesame'
priority: 'core'
---

# Sesame Seed - Mineral Density Layer

## Active Compounds

*   **Sesamin**
*   **Sesamol**
*   **Calcium**
*   **Magnesium**

## Mechanism of Action

Sesame:

*   **Provides mineral density**
*   **Influences lipid metabolism**
*   **Supports antioxidant status**

## Dose

*   1 tablespoon (10–15g)
*   **Rotated, not daily.**

## Protocol Role

*   **On non-cacao days**
*   **When magnesium density is desired**
*   **Not stacked with chia on same day**

## Stacking Rules

- Use sesame as part of the salad’s mineral and texture layer rather than as an isolated snack-like add-on.
- Rotate it with other seeds when the protocol calls for variety, but keep the role consistent.

## Evidence Status

- **General physiology:** The compound and mechanism descriptions on this page are used as directional context for how the ingredient may fit into the protocol.
- **Protocol-specific rules:** The exact dose, timing, pairings, weekly caps, and "mandatory" language on this page are operating rules specific to this manual.

## Used In

- [Consolidated Master Shopping List](../05-practical/shopping-list-master.md)
- [Week 4 Overview](../03-execution/week-04/README.md)
- [Daily Execution](../03-execution/README.md)

## Conflicts

- Use sesame as part of the mineral / texture layer instead of assuming any seed is identical in every context.
- On higher-oxalate days, keep the broader mineral-balance logic in view rather than treating sesame as decoration.


## Culinary Integration (Recipes)
This ingredient is functionally integrated into the following protocol meals:
- [14:30 Salad Layer (Seed Rotation)](../05-practical/daily-salad-layer.md#explicit-weekly-rotation-table)
- [The Vascular Podi (Gunpowder)](../05-practical/functional-indian-formats.md#3-therapeutic-podis--gunpowders-dry-matrix)

## Related Pages

- [Consolidated Master Shopping List](../05-practical/shopping-list-master.md)
- [Oxalate Physiology and Mineral Pairing](../04-advanced-control/oxalate-management.md)
- [Week 4 Overview](../03-execution/week-04/README.md)
- [Ingredient Conflict Matrix](../07-appendices/ingredient-conflict-matrix.md)
