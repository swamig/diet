...
for file_path in files:
    ...
    new_content = content.replace(
        '*   **1/4 to 1/2 cup Broccoli Microgreens** (Sulforaphane Engine) [4](../../07-appendices/master-citations.md#4-sulforaphane--inflammatory-control)',
        salad_replacement
    )

    # Update Macro Execution Section
    # We find "    *   **Dressing" and replace it with the new line + "    *   **Dressing"
    new_content = re.sub(r'(\s*)\*\s*\*\*Dressing', r'\1*   **Broccoli Microgreens (14:30):** ' + ('1/2 cup (Performance Nrf2 dose).' if day_type == 'heavy' else '1/4 cup (Clinical baseline Nrf2 dose).') + r'\n\1*   **Dressing', new_content)
